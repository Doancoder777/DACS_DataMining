 ...
 * FIXED VERSION: Now properly reads ItemsetTree format (transaction_id item_id [count])
 * Enhanced for organized output structure
 * 
 * @see FPTree
 * @see Itemset
 * @see Itemsets
 * @author Philippe Fournier-Viger, Fixed for ItemsetTree format
 */
public class AlgoFPGrowth {
	...
	public Itemsets runAlgorithm(String input, String output, double minsupp) throws FileNotFoundException, IOException {
		...
		// FIXED: Build FP-Tree using ItemsetTree format
	...
	/**
	 * FIXED: Build FP-Tree using ItemsetTree transaction map format
	 * @param input input file path
	 * @param tree FP-Tree to build
	 * @param mapSupport map of item frequencies
	...
	private void buildFPTreeFromTransactionMap(String input, FPTree tree, Map<Integer, Integer> mapSupport) throws IOException {
		...
		// Step 2: Process each transaction and add to FP-Tree
		...
		for (Map.Entry<Integer, List<Integer>> entry : transactionMap.entrySet()) {
			...
			// Filter transaction: keep only frequent items
	 ...
	 * @param prefixSupport the support of the current prefix
	 ...
	 * @param fpNodeTempBuffer the nodes in the prefix path
	 * @param position the number of nodes in the prefix path
...
loop1:	for (long i = 1, max = 1 << position; i < max; i++) {
	...
	/**
	 * FIXED: This method scans the input database to calculate the support of single items
	 * Using ItemsetTree format: transaction_id item_id [count]
	...
	private Map<Integer, Integer> scanDatabaseToDetermineFrequencyOfSingleItems(String input)
		...
		// Read first line (header with transaction count and item count)
		line = reader.readLine();
		if (line == null || line.trim().isEmpty()) {
			reader.close();
			throw new IOException("Input file is empty or invalid");
		...
		// Step 1: Read all transactions and count item frequency
		Map<Integer, List<Integer>> transactionMap = new HashMap<>();
		
		// Read lines to group items by transaction_id and count frequency
		while ((line = reader.readLine()) != null) {
			if (line.isEmpty() || line.charAt(0) == '#' || line.charAt(0) == '%' || line.charAt(0) == '@') {
				continue;
			}
			...
			if (count > 0) {
				// Add item to transaction (avoid duplicates in same transaction)
				transactionMap.computeIfAbsent(transactionId, k -> new ArrayList<>());
				if (!transactionMap.get(transactionId).contains(itemId)) {
					transactionMap.get(transactionId).add(itemId);
					
					// Increase frequency of item (count once per transaction)
					mapSupport.put(itemId, mapSupport.getOrDefault(itemId, 0) + 1);
				}
			}
		}
		
		// Set transaction count to number of unique transactions
